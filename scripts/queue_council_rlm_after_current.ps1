...
$outDir = Join-Path $Root "records\vocabmoe16-council-rlm-5k-auto-$stamp"
$logPath = Join-Path $Root "records\vocabmoe16-council-rlm-after-current-$stamp.queue.log"
...
Write-QueueLog "starting 16MB council/RLM-lite matrix out=$outDir"
$args = @(
...
Write-QueueLog "16MB council/RLM-lite matrix exited code=$exitCode out=$outDir"
exit $exitCode

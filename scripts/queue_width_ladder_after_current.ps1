...
$outDir = Join-Path $Root "records\sub4-width-ladder-i4-5k-auto-$stamp"
$logPath = Join-Path $Root "records\sub4-width-ladder-after-current-$stamp.queue.log"
...
Write-QueueLog "starting sub4 width-ladder matrix out=$outDir"
$args = @(
    "scripts\run_sub4_iotail_quant_matrix.py",
    "--candidate-group", "sub4_width_ladder",
    "--out", $outDir,
    "--iterations", "5000",
    "--warmdown-iters", "5000",
    "--val-tokens", "65536",
...
Write-QueueLog "sub4 width-ladder matrix exited code=$exitCode out=$outDir"
exit $exitCode
